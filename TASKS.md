...
- [ ] **16.1.8** Sidebar status pill driven by ConversationLoop transitions. The Phase 14.2 status pill exists but reads from the legacy daemon-up signal. Subscribe to `host.loop.onTransition` and broadcast a `{type:"loopState", state:"idle|listening|thinking|speaking|interrupted|quiet"}` WS event so the sidebar pill reflects the actual conversation state. The sidebar webview already has a state-pill DOM node from 14.2; just wire the new event type. Test: drive the loop through THINKING + SPEAKING; assert two `loopState` frames arrived in the right order.

- [ ] **16.2** `BUDDY_PROVIDER=ollama` crashes the daemon on boot. Empirical reproduction: starting the built `daemon/dist/index.js` with `BUDDY_PROVIDER=ollama` exits 1 with `Error: personality 'nsfw': config file missing at daemon/prompts/personalities/nsfw.json`. The personality `.md` is correctly gated (loaded only on Ollama via `personalities-loader.ts`'s ollama-only directory), but its required voice-config sidecar lives next to the *base* personalities (`daemon/prompts/personalities/*.json`) and was never shipped for `nsfw`. Fix one of: (a) commit `daemon/prompts/personalities/nsfw.json` with a sensible voice config; (b) make `loadPersonalityConfigs` treat missing config as an opt-out (skip the personality with a warn log) so the gating logic alone gates loading. Test: daemon boots cleanly with both `BUDDY_PROVIDER=anthropic` and `BUDDY_PROVIDER=ollama` against a fresh checkout.

- [ ] **16.3** Setup scripts don't actually download models. `setup.sh:161-168` and `setup.ps1:179-186` print `"Model-download step is a stub until Task 15.10 ships the manifest schema"` and skip the download branch — but Task 15.10 *did* ship the manifest. Compounding this, every `sha256` in `voice/models.json` is `"TBD-fill-in-when-downloading"`, and the daemon's verifier (`models-manifest.ts:155-159`) explicitly skips TBD entries — so the README claim "daemon refuses to start on checksum mismatch" is currently a no-op. Implement the download branch (read manifest, `curl -fL` / `Invoke-WebRequest` each entry to `path`, verify `size_bytes`, then SHA256 via `sha256sum -c`). Replace TBD hashes with real values. Test: a fresh checkout running `pnpm doctor` after `./setup.sh` passes the model-checksum check; integration test in CI exercises one model download (cached layer to keep CI fast).
...
- [ ] **16.6** Redactor coverage gaps. `extension/src/redactor.ts` and `daemon/src/redactor.ts` (kept in sync by the drift canary in `payload-assembler.test.mjs:164-191`, verified working) both miss:
